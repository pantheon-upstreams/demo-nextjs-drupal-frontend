...
export function Navigation() {
  ...
  // Fallback nav, used ONLY when the Drupal `nextjs` menu returns no items
  // (e.g. Drupal unreachable or the menu hasn't been configured).
  const staticLinks = [
  ...
  // The Drupal `nextjs` menu drives navigation when present; otherwise fall back.
  const allNavLinks = menuItems.length > 0
    ? menuItems.map(item => ({
        href: item.url,
        label: item.title,
  ...
  return (
    <>
      ...
      <nav className="fixed top-0 w-full z-50 bg-background border-b border-border shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-4">
        <div className="flex justify-between items-center">
          <Link href="/" className="flex items-center" aria-label={config.site.name}>
            {/* Official Pantheon logo (wordmark + lightning bolt). White variant in dark mode. */}
            <img src="/pantheon-logo.svg" alt="Pantheon" className="h-8 w-auto dark:hidden" />
            <img src="/pantheon-logo-white.svg" alt="Pantheon" className="h-8 w-auto hidden dark:block" />
            <span className="sr-only">{config.site.name}</span>
          </Link>
