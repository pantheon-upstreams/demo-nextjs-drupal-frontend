...
function processDirectImageField(
  ...
  const baseUrl = process.env.NEXT_PUBLIC_DRUPAL_BASE_URL || 'http://localhost:8080';
    ...
    relationships.field_tags.data.forEach(tagRef => {
      if (included) {
        ...
        if (tagEntity && tagEntity.attributes) {
          tags.push({
            id: tagEntity.id,
            name: (tagEntity.attributes as unknown as { name: string }).name
          });
        }
      }
    });
...
export async function getAllPagesFromDrupal(): Promise<Page[]> {
  try {
    ...
    console.warn('Pages: Unable to fetch pages from Drupal:', error instanceof Error ? error.message : error);
    return [];
  }
}
...
export async function getPageFromDrupal(slugOrPath: string): Promise<Page | null> {
  try {
    ...
    console.warn(`Page: Unable to fetch page '${slugOrPath}' from Drupal:`, error instanceof Error ? error.message : error);
    return null;
  }
}
